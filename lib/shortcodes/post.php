 ...
 *
 * @package Power\Shortcodes
 * @author  Core Engine
 * @license GPL-2.0-or-later
 * @link    www.daniellane.eu
 */

add_shortcode( 'post_date', 'power_post_date_shortcode' );
/**
 ...
 *
 * Output passes through `power_post_date_shortcode` filter before returning.
 *
 * @since 1.1.0
 *
 ...
 */
function power_post_date_shortcode( $atts ) {
	...
	if ( 'relative' === $atts['format'] ) {
		$display  = power_human_time_diff( get_the_time( 'U' ), current_time( 'timestamp' ), $atts['relative_depth'] );
		$display .= ' ' . __( 'ago', 'power' );
	} else {
		$display = get_the_time( $atts['format'] );
	}

	$output = sprintf( '<time %s>', power_attr( 'entry-time' ) ) . $atts['before'] . $atts['label'] . $display . $atts['after'] . '</time>';

	return apply_filters( 'power_post_date_shortcode', $output, $atts );

}

add_shortcode( 'post_time', 'power_post_time_shortcode' );
/**
 ...
 *   label (text following 'before' output, but before date).
 *
 * Output passes through `power_post_time_shortcode` filter before returning.
 *
 * @since 1.1.0
 *
 * @param array|string $atts Shortcode attributes. Empty string if no attributes.
 ...
 */
function power_post_time_shortcode( $atts ) {
	...
	$output = sprintf( '<time %s>', power_attr( 'entry-time' ) ) . $atts['before'] . $atts['label'] . get_the_time( $atts['format'] ) . $atts['after'] . '</time>';

	return apply_filters( 'power_post_time_shortcode', $output, $atts );

}

add_shortcode( 'post_modified_date', 'power_post_modified_date_shortcode' );
/**
 ...
 *  * label (text following 'before' output, but before date).
 *
 * Output passes through `power_post_modified_date_shortcode` filter before returning.
 *
 * @since 2.1.0
 *
 * @param array|string $atts Shortcode attributes. Empty string if no attributes.
 ...
 */
function power_post_modified_date_shortcode( $atts ) {
	...
	if ( 'relative' === $atts['format'] ) {
		$display  = power_human_time_diff( get_the_modified_time( 'U' ), current_time( 'timestamp' ), $atts['relative_depth'] );
		$display .= ' ' . __( 'ago', 'power' );
	} else {
		$display = get_the_modified_time( $atts['format'] );
	}

	$output = sprintf( '<time %s>', power_attr( 'entry-modified-time' ) ) . $atts['before'] . $atts['label'] . $display . $atts['after'] . '</time>';
	 ...
	 * }
	 */
	return apply_filters( 'power_post_modified_date_shortcode', $output, $atts );

}

add_shortcode( 'post_modified_time', 'power_post_modified_time_shortcode' );
/**
 ...
 *
 * Output passes through `power_post_modified_time_shortcode` filter before returning.
 *
 * @since 2.1.0
 *
 * @param array|string $atts Shortcode attributes. Empty string if no attributes.
 ...
 */
function power_post_modified_time_shortcode( $atts ) {
	...
	$output = sprintf( '<time %s>', power_attr( 'entry-modified-time' ) ) . $atts['before'] . $atts['label'] . get_the_modified_time( $atts['format'] ) . $atts['after'] . '</time>';
	 ...
	 * }
	 */
	return apply_filters( 'power_post_modified_time_shortcode', $output, $atts );

}

add_shortcode( 'post_author', 'power_post_author_shortcode' );
/**
 ...
 *   before (output before link, default is empty string).
 *
 * Output passes through `power_post_author_shortcode` filter before returning.
 *
 * @since 1.1.0
 *
 * @param array|string $atts Shortcode attributes. Empty string if no attributes.
 ...
 */
function power_post_author_shortcode( $atts ) {
	...
	$output  = sprintf( '<span %s>', power_attr( 'entry-author' ) );
	$output .= $atts['before'];
	$output .= sprintf( '<span %s>', power_attr( 'entry-author-name' ) ) . esc_html( $author ) . '</span>';
	$output .= $atts['after'];
	$output .= '</span>';

	return apply_filters( 'power_post_author_shortcode', $output, $atts );

}

add_shortcode( 'post_author_link', 'power_post_author_link_shortcode' );
/**
 ...
 *   before (output before link, default is empty string).
 *
 * Output passes through `power_post_author_link_shortcode` filter before returning.
 *
 * @since 1.1.0
 *
 * @param array|string $atts Shortcode attributes. Empty string if no attributes.
 * @return string Return empty string if post type does not support `author` or post has no author assigned.
 *                Return `power_post_author_shortcode()` if author has no URL.
 *                Otherwise, output for `post_author_link` shortcode.
 */
function power_post_author_link_shortcode( $atts ) {
	...
	if ( ! $url ) {
		return power_post_author_shortcode( $atts );
	}
	...
	$output  = sprintf( '<span %s>', power_attr( 'entry-author' ) );
	$output .= $atts['before'];
	$output .= sprintf( '<a href="%s" %s>', $url, power_attr( 'entry-author-link' ) );
	$output .= sprintf( '<span %s>', power_attr( 'entry-author-name' ) );
	$output .= esc_html( $author );
	$output .= '</span></a>' . $atts['after'] . '</span>';

	return apply_filters( 'power_post_author_link_shortcode', $output, $atts );

}

add_shortcode( 'post_author_posts_link', 'power_post_author_posts_link_shortcode' );
/**
 ...
 *   before (output before link, default is empty string).
 *
 * Output passes through `power_post_author_posts_link_shortcode` filter before returning.
 *
 * @since 1.1.0
 *
 * @param array|string $atts Shortcode attributes. Empty string if no attributes.
 ...
 */
function power_post_author_posts_link_shortcode( $atts ) {
	...
	return apply_filters( 'power_post_author_posts_link_shortcode', $output, $atts );

}

add_shortcode( 'post_comments', 'power_post_comments_shortcode' );
/**
 ...
 *
 * Output passes through `power_post_comments_shortcode` filter before returning.
 *
 * @since 1.1.0
 *
 * @param array|string $atts Shortcode attributes. Empty string if no attributes.
 * @return string Return empty string if post does not support `comments`, or `hide_if_off` is enabled and
 *                comments are closed or disabled in Power theme settings.
 *                Otherwise, output for `post_comments` shortcode.
 */
function power_post_comments_shortcode( $atts ) {
	...
	$defaults = [
		...
		'before'      => '',
		'hide_if_off' => 'enabled',
		'more'        => __( '% Comments', 'power' ),
		'one'         => __( '1 Comment', 'power' ),
		'zero'        => __( 'Leave a Comment', 'power' ),
	];
	$atts     = shortcode_atts( $defaults, $atts, 'post_comments' );

	if ( 'enabled' === $atts['hide_if_off'] && ( ! power_get_option( 'comments_posts' ) || ! comments_open() ) ) {
		return '';
	}
	...
	return apply_filters( 'power_post_comments_shortcode', $output, $atts );

}

add_shortcode( 'post_tags', 'power_post_tags_shortcode' );
/**
 ...
 *
 * Output passes through `power_post_tags_shortcode` filter before returning.
 *
 * @since 1.1.0
 *
 * @param array|string $atts Shortcode attributes. Empty string if no attributes.
 ...
 */
function power_post_tags_shortcode( $atts ) {
	...
	$output = sprintf( '<span %s>', power_attr( 'entry-tags' ) ) . $tags . '</span>';

	return apply_filters( 'power_post_tags_shortcode', $output, $atts );

}

add_shortcode( 'post_categories', 'power_post_categories_shortcode' );
/**
 ...
 *   sep (separator string between tags, default is ', ').
 *
 * Output passes through 'power_post_categories_shortcode' filter before returning.
 *
 * @since 1.1.0
 *
 * @param array|string $atts Shortcode attributes. Empty string if no attributes.
 ...
 */
function power_post_categories_shortcode( $atts ) {
	...
	$output = sprintf( '<span %s>', power_attr( 'entry-categories' ) ) . $atts['before'] . $cats . $atts['after'] . '</span>';

	return apply_filters( 'power_post_categories_shortcode', $output, $atts );

}

add_shortcode( 'post_terms', 'power_post_terms_shortcode' );
/**
 ...
 *
 * Output passes through `power_post_terms_shortcode` filter before returning.
 *
 * @since 1.6.0
 *
 * @param array|string $atts Shortcode attributes. Empty string if no attributes.
 ...
 */
function power_post_terms_shortcode( $atts ) {
	 ...
	 */
	$defaults = apply_filters( 'power_post_terms_shortcode_defaults', $defaults );
	...
	$output = sprintf( '<span %s>', power_attr( 'entry-terms' ) ) . $terms . '</span>';

	return apply_filters( 'power_post_terms_shortcode', $output, $terms, $atts );

}

add_shortcode( 'post_edit', 'power_post_edit_shortcode' );
/**
 ...
 *
 * Output passes through `power_post_edit_shortcode` filter before returning.
 *
 * @since 1.1.0
 *
 * @param array|string $atts Shortcode attributes. Empty string if no attributes.
 * @return string Output for `post_edit` shortcode, or empty string if `power_edit_post_link` filter returns `false`.
 */
function power_post_edit_shortcode( $atts ) {

	if ( ! apply_filters( 'power_edit_post_link', true ) ) {
		return '';
	}
	...
	return apply_filters( 'power_post_edit_shortcode', $output, $atts );
