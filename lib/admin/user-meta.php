...
add_action( 'show_user_profile', 'power_user_options_fields' );
add_action( 'edit_user_profile', 'power_user_options_fields' );
/**
 * Add fields for user permissions for Power features to the user edit screen.
 *
 * Checkbox settings are:
 *
 * * Enable Power Admin Menu?
...
add_action( 'show_user_profile', 'power_user_archive_fields' );
add_action( 'edit_user_profile', 'power_user_archive_fields' );
/**
 ...
 * @return void Return early if current user can not edit users.
 */
function power_user_archive_fields( $user ) {

	if ( current_user_can( 'edit_users', $user->ID ) ) {
		power_meta_boxes()->show_meta_box( 'power-user-author', $user );
	}

}

add_action( 'show_user_profile', 'power_user_seo_fields' );
add_action( 'edit_user_profile', 'power_user_seo_fields' );
/**
...
add_action( 'show_user_profile', 'power_user_layout_fields' );
add_action( 'edit_user_profile', 'power_user_layout_fields' );
/**
 ...
 * @param \WP_User $user User object.
 *
 * @return void Return early if current theme does not support `power-archive-layouts`, or Power
...
add_action( 'personal_options_update', 'power_user_meta_save' );
add_action( 'edit_user_profile_update', 'power_user_meta_save' );
/**
...
add_filter( 'get_the_author_power_admin_menu', 'power_user_meta_default_on', 10, 2 );
add_filter( 'get_the_author_power_seo_settings_menu', 'power_user_meta_default_on', 10, 2 );
add_filter( 'get_the_author_power_import_export_menu', 'power_user_meta_default_on', 10, 2 );
/**
...
add_filter( 'get_the_author_power_author_box_single', 'power_author_box_single_default_on', 10, 2 );
/**
