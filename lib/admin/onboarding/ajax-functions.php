...
add_action( 'wp_ajax_power_do_onboarding_process', 'power_do_onboarding_process' );
/**
 * Processes onboarding tasks in batches.
 *
 * @since 2.8.0
 */
function power_do_onboarding_process() {

	check_ajax_referer( 'power-onboarding', 'nonce' );
	...
	if ( 'dependencies' === $task ) {
		...
		$onboarding_plugins       = power_onboarding_plugins();
		...
		$installed = power_onboarding_install_dependencies( $onboarding_plugins, $step );
	...
	if ( 'content' === $task ) {
		...
		$imported = power_onboarding_import_content( $content );

		if ( ! empty( $imported['errors'] ) ) {
			$errors[] = $imported['errors'];
		}

		$menus = power_onboarding_create_navigation_menus();

		if ( ! empty( $menus ) ) {
			$errors[] = $menus;
		}

		$menu_items = power_onboarding_create_navigation_menu_items();
...
add_action( 'wp_ajax_power_do_onboarding_pack_selection', 'power_do_onboarding_pack_selection' );
/**
 * Set the chosen Starter Pack.
 *
 * Temporarily store the chosen starter pack in a `power_onboarding_chosen_pack`
 ...
 * @since 3.1.0
 */
function power_do_onboarding_pack_selection() {
	...
	update_option( 'power_onboarding_chosen_pack', $pack, false );

	$menus = power_onboarding_navigation_menus();

	wp_send_json_success(
		[
			'tasks'    => power_onboarding_tasks(),
