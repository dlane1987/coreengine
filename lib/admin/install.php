...
namespace CoreEngine\Power\Admin\Install;

\add_action( 'after_switch_theme', __NAMESPACE__ . '\\insert_default_settings' );
/**
 * Inserts the default Power settings values into the options table, if they don't already exist.
 *
 * @since 3.0.3
 *
...
function insert_default_settings() {
	return add_option(
		...
		[
			...
			'header_scripts'            => '',
			'footer_scripts'            => '',
			'footer_text'               => sprintf( '[footer_copyright before="%s "] · [footer_childtheme_link before="" after=" %s"] [footer_power_link url="https://www.coreengine.com/" before=""] · [footer_wordpress_link] · [footer_loginout]', __( 'Copyright', 'power' ), __( 'on', 'power' ) ),
			'theme_version'             => PARENT_THEME_VERSION,
			'db_version'                => PARENT_DB_VERSION,
			'first_version'             => power_first_version(),
		],
		'',
		true
	);
