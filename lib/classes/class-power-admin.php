...
abstract class Power_Admin {
	...
	public function create( $page_id = '', array $menu_ops = [], array $page_ops = [], $settings_field = '', array $default_settings = [] ) {
		...
		$this->help_base        = $this->help_base ?: POWER_VIEWS_DIR . '/help/' . $page_id . '-';
		$this->views_base       = $this->views_base ?: POWER_VIEWS_DIR;

		$this->page_ops = wp_parse_args(
			$this->page_ops,
			[
				'save_button_text'  => __( 'Save Changes', 'power' ),
				'reset_button_text' => __( 'Reset Settings', 'power' ),
				'saved_notice_text' => __( 'Settings saved.', 'power' ),
				'reset_notice_text' => __( 'Settings reset.', 'power' ),
				'error_notice_text' => __( 'Error saving settings.', 'power' ),
			]
		);
		...
		if ( isset( $this->menu_ops['submenu'] ) && ( isset( $this->menu_ops['main_menu'] ) || isset( $this->menu_ops['first_submenu'] ) ) ) {
			/* translators: %s: Power_Admin class name. */
			wp_die(
				sprintf(
					/* translators: %s: Power_Admin class name. */
					esc_html__( 'You cannot use %s to create two menus in the same subclass. Please use separate subclasses for each menu.', 'power' ),
					'Power_Admin'
				)
			);
		}
	...
	public function maybe_add_main_menu() {
		...
		if ( isset( $this->menu_ops['main_menu']['sep'] ) ) {
			...
			if ( $sep['sep_position'] && $sep['sep_capability'] ) {
				$GLOBALS['menu'][ $sep['sep_position'] ] = [ '', $sep['sep_capability'], 'separator', '', 'power-separator wp-menu-separator' ]; // phpcs:ignore WordPress.WP.GlobalVariablesOverride.Prohibited -- Intentionally overriding the global here.
			}
		}
	...
	public function maybe_redirect() {
		...
		if ( ! power_is_menu_page( $this->page_id ) ) {
			return;
		}
	...
	public function register_settings() {
		...
		if ( ! power_get_option( 'theme_version' ) ) {
		...
		if ( power_get_option( 'reset', $this->settings_field ) ) {
	...
	/**
	 * Set help sidebar for Power screens.
	 *
	 * @since 2.5.0
	 */
	public function set_help_sidebar() {
		...
		$screen_reader = '<span class="screen-reader-text">. ' . esc_html__( 'Link opens in a new window.', 'power' ) . '</span>';
		$current_screen->set_help_sidebar(
			'<p><strong>' . esc_html__( 'For more information:', 'power' ) . '</strong></p>' .
			'<p><a href="http://my.coreengine.com/help/" target="_blank" rel="noopener noreferrer">' . esc_html__( 'Get Support', 'power' ) . $screen_reader . '</a></p>' .
			'<p><a href="http://my.coreengine.com/snippets/" target="_blank" rel="noopener noreferrer">' . esc_html__( 'Power Snippets', 'power' ) . $screen_reader . '</a></p>' .
			'<p><a href="http://my.coreengine.com/tutorials/" target="_blank" rel="noopener noreferrer">' . esc_html__( 'Power Tutorials', 'power' ) . $screen_reader . '</a></p>'
		);
	 ...
	 *
	 * Within Power pages, the id attributes of form fields are the same as
	...
	protected function get_field_value( $key ) {
		...
		return power_get_option( $key, $this->settings_field );
