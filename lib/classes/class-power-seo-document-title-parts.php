...
/**
 * Generates the SEO Document Title for Power.
 *
 * @since 2.6.0
 *
 * @package Power\SEO
 */
class Power_SEO_Document_Title_Parts {
	...
	public function get_parts( array $parts = [] ) {
		...
		if ( ! power_get_seo_option( 'append_site_title' ) ) {
	...
	public function get_root_page_title_parts() {
		...
		$home_doctitle      = power_get_seo_option( 'home_doctitle' );
		$append_description = power_get_seo_option( 'append_description_home' );
	...
	public function get_singular_title_parts( $post_id = null ) {
		...
		foreach ( $supported_title_keys as $key ) {
			if ( power_get_custom_field( $key, $post_id ) ) {
				$this->parts[ self::TITLE ] = power_get_custom_field( $key, $post_id );
				break;
			}
		}
	...
	public function get_post_type_archive_title_parts() {
		...
		if ( power_has_post_type_archive_support() ) {

			$cpt_title = power_get_cpt_option( 'doctitle' );
