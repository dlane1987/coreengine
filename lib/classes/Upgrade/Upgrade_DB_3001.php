...
/**
 * Upgrade class. Called when `db_version` Power setting is below 3001.
 *
 * @since 3.1.0
 */
class Upgrade_DB_3001 implements Upgrade_DB_Interface {
	...
	public function migrate_blog_pages() {
		...
		$parameters = array_filter(
			[
				'cat'              => power_get_option( 'blog_cat', POWER_SETTINGS_FIELD, false ),
				'category__not_in' => power_get_option( 'blog_cat_exclude', POWER_SETTINGS_FIELD, false ),
				'showposts'        => power_get_option( 'blog_cat_num', POWER_SETTINGS_FIELD, false ),
			]
		);

		foreach ( $page_blog_ids as $page_blog_id ) {
			$existing_query_args = power_get_custom_field( 'query_args', $page_blog_id );
...
add_action( 'power_entry_content', 'power_page_archive_content' );

function power_page_archive_content() {
    $heading = ( power_a11y( 'headings' ) ? 'h2' : 'h4' );

    power_sitemap( $heading );
}
