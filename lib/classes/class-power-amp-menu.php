...
class Power_AMP_Menu {
	...
	public function add_hooks() {
		...
		foreach ( $this->script_config['menuClasses'] as $combine_or_other ) {
			foreach ( $combine_or_other as $theme_location ) {
				...
				add_filter( "power_attr_{$theme_location}", [ $this, 'add_nav_class_attribute' ] );
			}
		}

		add_filter( "power_{$this->hamburger_config['filter_location']}", [ $this, 'add_hamburger_button' ] );
		add_filter( "power_attr_nav-{$this->hamburger_config['theme_location']}-toggle", [ $this, 'add_hamburger_attributes' ] );
		add_filter( "power_attr_nav-{$this->hamburger_config['theme_location']}", [ $this, 'add_hamburger_target_menu_attributes' ] );
		add_filter( 'walker_nav_menu_start_el', [ $this, 'add_nav_submenu_toggle' ], 10, 4 );
		add_filter( 'power_attr_sub-menu-toggle', [ $this, 'add_submenu_toggle_attributes' ], 10, 3 );
		add_action( 'power_after', [ $this, 'output_submenu_amp_states' ] );
	...
	public function add_custom_css() {
		...
		$css = sprintf(
			'
			/* Power+AMP responsive menu styles.
			--------------------------------------------- */
			@media only screen and (min-width: %1$s) {
				.menu-item.power-amp-combined { display: none }
			}

			@media only screen and (max-width: %1$s) {
				.header-menu .power-responsive-menu,
				.power-responsive-menu {
					display: block;
					position: absolute;
					left: -9999px;
					opacity: 0;
				...
				.power-nav-menu.responsive-menu.toggled-on,
				.power-responsive-menu.toggled-on {
					opacity: 1;
					position: relative;
					left: auto;
				...
				.power-responsive-menu.toggled-on .menu-item .sub-menu,
				.power-responsive-menu.toggled-on .menu-item:hover > .sub-menu {
					display: none;
				}

				.power-responsive-menu.toggled-on .sub-menu-toggle.toggled-on + .sub-menu {
					display: block;
					width: 100%;
					-webkit-transform: scaleY(1);
					-moz-transform: scaleY(1);
		...
		wp_add_inline_style( power_get_theme_handle(), $css );

	}

	/**
	 * Add 'power-responsive-menu' class attribute to the `<nav>` element.
	 *
	 * @since 0.1.0
	 *
	...
	public function add_nav_class_attribute( array $attributes ) {
		...
		$attributes['class'] .= ' power-responsive-menu';
	...
	public function add_hamburger_attributes( array $attributes ) {
		...
		$attributes['id']            = 'power-mobile-nav-' . esc_attr( $this->hamburger_config['theme_location'] );
	...
	public function add_nav_submenu_toggle( $item_output, $item, $depth, $args ) {
		...
		$item_output .= power_markup(
			[
				'open'           => '<button %s>',
				'close'          => '</button>',
				'context'        => 'sub-menu-toggle',
				'content'        => sprintf( '<span class="screen-reader-text">%s</span>', esc_html__( 'Submenu', 'power' ) ),
				'echo'           => false,
				'state_id'       => $state_id,
