...
/**
 * Power Featured Post widget class.
 *
 * @since 1.0.0
 *
 * @package Power\Widgets
 */
class Power_Featured_Post extends WP_Widget {
	...
	public function __construct() {
		...
		$this->defaults = [
			...
			'show_title'              => 0,
			'show_byline'             => 0,
			'post_info'               => '[post_date] ' . __( 'By', 'power' ) . ' [post_author_posts_link] [post_comments]',
			'show_content'            => 'excerpt',
			'content_limit'           => '',
			'more_text'               => __( '[Read More...]', 'power' ),
			'extra_num'               => '',
			'extra_title'             => '',
			'more_from_category'      => '',
			'more_from_category_text' => __( 'More Posts from this Category', 'power' ),
		];

		$widget_ops = [
			'classname'   => 'featured-content featuredpost',
			'description' => __( 'Displays featured posts with thumbnails', 'power' ),
		];
		...
		parent::__construct( 'featured-post', __( 'Power - Featured Posts', 'power' ), $widget_ops, $control_ops );
	...
	public function widget( $args, $instance ) {
		...
		global $wp_query, $_power_displayed_ids;
		...
		if ( $instance['exclude_displayed'] ) {
			$query_args['post__not_in'] = (array) $_power_displayed_ids;
		}
			...
			while ( have_posts() ) {
				...
				$image = power_get_image(
					[
						...
						'attr'    => power_parse_attr( 'entry-image-widget', [] ),
					]
				);
				...
				if ( $instance['show_title'] || $instance['show_byline'] ) {
					...
					if ( ! empty( $instance['show_title'] ) ) {
						...
						$title = get_the_title() ?: __( '(no title)', 'power' );
						 ...
						 * }
						 */
						$title   = apply_filters( 'power_featured_post_title', $title, $instance, $args );
						$heading = power_a11y( 'headings' ) ? 'h4' : 'h2';
					...
					if ( ! empty( $instance['show_byline'] ) && ! empty( $instance['post_info'] ) ) {
						$header .= power_markup(
							[
								...
								'close'   => '</p>',
								'context' => 'entry-meta',
								'content' => power_strip_p_tags( do_shortcode( $instance['post_info'] ) ),
								'params'  => [
									'is_widget' => true,
								],
								'echo'    => false,
				...
				if ( ! empty( $instance['show_content'] ) ) {
					...
					if ( 'excerpt' === $instance['show_content'] ) {
						...
						the_content_limit( (int) $instance['content_limit'], power_a11y_more_link( esc_html( $instance['more_text'] ) ) );
					} else {
						global $more;
						...
						the_content( power_a11y_more_link( esc_html( $instance['more_text'] ) ) );
	...
	public function form( $instance ) {
		...
		?>
		<p>
			<label for="<?php echo esc_attr( $this->get_field_id( 'title' ) ); ?>"><?php esc_html_e( 'Title', 'power' ); ?>:</label>
			<input type="text" id="<?php echo esc_attr( $this->get_field_id( 'title' ) ); ?>" name="<?php echo esc_attr( $this->get_field_name( 'title' ) ); ?>" value="<?php echo esc_attr( $instance['title'] ); ?>" class="widefat" />
		</p>

		<div class="power-widget-column">

			<div class="power-widget-column-box power-widget-column-box-top">

				<p>
					<label for="<?php echo esc_attr( $this->get_field_id( 'posts_cat' ) ); ?>"><?php esc_html_e( 'Category', 'power' ); ?>:</label>
					<?php
					$categories_args = [
						...
						'orderby'         => 'Name',
						'hierarchical'    => 1,
						'show_option_all' => __( 'All Categories', 'power' ),
						'hide_empty'      => '0',
					];
				...
				<p>
					<label for="<?php echo esc_attr( $this->get_field_id( 'posts_num' ) ); ?>"><?php esc_html_e( 'Number of Posts to Show', 'power' ); ?>:</label>
				...
				<p>
					<label for="<?php echo esc_attr( $this->get_field_id( 'posts_offset' ) ); ?>"><?php esc_html_e( 'Number of Posts to Offset', 'power' ); ?>:</label>
				...
				<p>
					<label for="<?php echo esc_attr( $this->get_field_id( 'orderby' ) ); ?>"><?php esc_html_e( 'Order By', 'power' ); ?>:</label>
					<select id="<?php echo esc_attr( $this->get_field_id( 'orderby' ) ); ?>" name="<?php echo esc_attr( $this->get_field_name( 'orderby' ) ); ?>">
						<option value="date" <?php selected( 'date', $instance['orderby'] ); ?>><?php esc_html_e( 'Date Published', 'power' ); ?></option>
						<option value="modified" <?php selected( 'modified', $instance['orderby'] ); ?>><?php esc_html_e( 'Date Modified', 'power' ); ?></option>
						<option value="title" <?php selected( 'title', $instance['orderby'] ); ?>><?php esc_html_e( 'Title', 'power' ); ?></option>
						<option value="parent" <?php selected( 'parent', $instance['orderby'] ); ?>><?php esc_html_e( 'Parent', 'power' ); ?></option>
						<option value="ID" <?php selected( 'ID', $instance['orderby'] ); ?>><?php esc_html_e( 'ID', 'power' ); ?></option>
						<option value="comment_count" <?php selected( 'comment_count', $instance['orderby'] ); ?>><?php esc_html_e( 'Comment Count', 'power' ); ?></option>
						<option value="rand" <?php selected( 'rand', $instance['orderby'] ); ?>><?php esc_html_e( 'Random', 'power' ); ?></option>
					</select>
				</p>

				<p>
					<label for="<?php echo esc_attr( $this->get_field_id( 'order' ) ); ?>"><?php esc_html_e( 'Sort Order', 'power' ); ?>:</label>
					<select id="<?php echo esc_attr( $this->get_field_id( 'order' ) ); ?>" name="<?php echo esc_attr( $this->get_field_name( 'order' ) ); ?>">
						<option value="DESC" <?php selected( 'DESC', $instance['order'] ); ?>><?php esc_html_e( 'Descending (3, 2, 1)', 'power' ); ?></option>
						<option value="ASC" <?php selected( 'ASC', $instance['order'] ); ?>><?php esc_html_e( 'Ascending (1, 2, 3)', 'power' ); ?></option>
					</select>
				</p>
				...
				<p>
					...
					<label for="<?php echo esc_attr( $this->get_field_id( 'exclude_displayed' ) ); ?>"><?php esc_html_e( 'Exclude Previously Displayed Posts?', 'power' ); ?></label>
				</p>
				...
				<p>
					...
					<label for="<?php echo esc_attr( $this->get_field_id( 'exclude_sticky' ) ); ?>"><?php esc_html_e( 'Exclude Sticky Posts?', 'power' ); ?></label>
				</p>

			</div>

			<div class="power-widget-column-box">
				...
				<p>
					...
					<label for="<?php echo esc_attr( $this->get_field_id( 'show_gravatar' ) ); ?>"><?php esc_html_e( 'Show Author Gravatar', 'power' ); ?></label>
				</p>

				<p>
					<label for="<?php echo esc_attr( $this->get_field_id( 'gravatar_size' ) ); ?>"><?php esc_html_e( 'Gravatar Size', 'power' ); ?>:</label>
					<select id="<?php echo esc_attr( $this->get_field_id( 'gravatar_size' ) ); ?>" name="<?php echo esc_attr( $this->get_field_name( 'gravatar_size' ) ); ?>">
						<option value="45" <?php selected( 45, $instance['gravatar_size'] ); ?>><?php esc_html_e( 'Small (45px)', 'power' ); ?></option>
						<option value="65" <?php selected( 65, $instance['gravatar_size'] ); ?>><?php esc_html_e( 'Medium (65px)', 'power' ); ?></option>
						<option value="85" <?php selected( 85, $instance['gravatar_size'] ); ?>><?php esc_html_e( 'Large (85px)', 'power' ); ?></option>
						<option value="125" <?php selected( 125, $instance['gravatar_size'] ); ?>><?php esc_html_e( 'Extra Large (125px)', 'power' ); ?></option>
					</select>
				</p>

				<p>
					<label for="<?php echo esc_attr( $this->get_field_id( 'gravatar_alignment' ) ); ?>"><?php esc_html_e( 'Gravatar Alignment', 'power' ); ?>:</label>
					<select id="<?php echo esc_attr( $this->get_field_id( 'gravatar_alignment' ) ); ?>" name="<?php echo esc_attr( $this->get_field_name( 'gravatar_alignment' ) ); ?>">
						<option value="alignnone">- <?php esc_html_e( 'None', 'power' ); ?> -</option>
						<option value="alignleft" <?php selected( 'alignleft', $instance['gravatar_alignment'] ); ?>><?php esc_html_e( 'Left', 'power' ); ?></option>
						<option value="alignright" <?php selected( 'alignright', $instance['gravatar_alignment'] ); ?>><?php esc_html_e( 'Right', 'power' ); ?></option>
					</select>
				</p>
			...
			<div class="power-widget-column-box">
				...
				<p>
					...
					<label for="<?php echo esc_attr( $this->get_field_id( 'show_image' ) ); ?>"><?php esc_html_e( 'Show Featured Image', 'power' ); ?></label>
				</p>

				<p>
					<label for="<?php echo esc_attr( $this->get_field_id( 'image_size' ) ); ?>"><?php esc_html_e( 'Image Size', 'power' ); ?>:</label>
					<select id="<?php echo esc_attr( $this->get_field_id( 'image_size' ) ); ?>" class="power-image-size-selector" name="<?php echo esc_attr( $this->get_field_name( 'image_size' ) ); ?>">
						<?php
				...
				<p>
					<label for="<?php echo esc_attr( $this->get_field_id( 'image_alignment' ) ); ?>"><?php esc_html_e( 'Image Alignment', 'power' ); ?>:</label>
					<select id="<?php echo esc_attr( $this->get_field_id( 'image_alignment' ) ); ?>" name="<?php echo esc_attr( $this->get_field_name( 'image_alignment' ) ); ?>">
						<option value="alignnone">- <?php esc_html_e( 'None', 'power' ); ?> -</option>
						<option value="alignleft" <?php selected( 'alignleft', $instance['image_alignment'] ); ?>><?php esc_html_e( 'Left', 'power' ); ?></option>
						<option value="alignright" <?php selected( 'alignright', $instance['image_alignment'] ); ?>><?php esc_html_e( 'Right', 'power' ); ?></option>
						<option value="aligncenter" <?php selected( 'aligncenter', $instance['image_alignment'] ); ?>><?php esc_html_e( 'Center', 'power' ); ?></option>
					</select>
				</p>
		...
		<div class="power-widget-column power-widget-column-right">
			...
			<div class="power-widget-column-box power-widget-column-box-top">
				...
				<p>
					...
					<label for="<?php echo esc_attr( $this->get_field_id( 'show_title' ) ); ?>"><?php esc_html_e( 'Show Post Title', 'power' ); ?></label>
				</p>
				...
				<p>
					...
					<label for="<?php echo esc_attr( $this->get_field_id( 'show_byline' ) ); ?>"><?php esc_html_e( 'Show Post Info', 'power' ); ?></label>
					...
					<label for="<?php echo esc_attr( $this->get_field_id( 'post_info' ) ); ?>" class="screen-reader-text"><?php esc_html_e( 'Content Post Info', 'power' ); ?></label>

				</p>

				<p>
					<label for="<?php echo esc_attr( $this->get_field_id( 'show_content' ) ); ?>"><?php esc_html_e( 'Content Type', 'power' ); ?>:</label>
					<select id="<?php echo esc_attr( $this->get_field_id( 'show_content' ) ); ?>" name="<?php echo esc_attr( $this->get_field_name( 'show_content' ) ); ?>">
						<option value="content" <?php selected( 'content', $instance['show_content'] ); ?>><?php esc_html_e( 'Show Content', 'power' ); ?></option>
						<option value="excerpt" <?php selected( 'excerpt', $instance['show_content'] ); ?>><?php esc_html_e( 'Show Excerpt', 'power' ); ?></option>
						<option value="content-limit" <?php selected( 'content-limit', $instance['show_content'] ); ?>><?php esc_html_e( 'Show Content Limit', 'power' ); ?></option>
						<option value="" <?php selected( '', $instance['show_content'] ); ?>><?php esc_html_e( 'No Content', 'power' ); ?></option>
					</select>
					<br />
					<label for="<?php echo esc_attr( $this->get_field_id( 'content_limit' ) ); ?>"><?php esc_html_e( 'Limit content to', 'power' ); ?>
						...
						<?php esc_html_e( 'characters', 'power' ); ?>
					</label>
				</p>

				<p>
					<label for="<?php echo esc_attr( $this->get_field_id( 'more_text' ) ); ?>"><?php esc_html_e( 'More Text (if applicable)', 'power' ); ?>:</label>
			...
			<div class="power-widget-column-box">
				...
				<p id="<?php echo esc_attr( $this->get_field_id( 'extra_title' ) ); ?>-descr"><?php esc_html_e( 'To display an unordered list of more posts from this category, please fill out the information below', 'power' ); ?>:</p>

				<p>
					<label for="<?php echo esc_attr( $this->get_field_id( 'extra_title' ) ); ?>"><?php esc_html_e( 'Title', 'power' ); ?>:</label>
				...
				<p>
					<label for="<?php echo esc_attr( $this->get_field_id( 'extra_num' ) ); ?>"><?php esc_html_e( 'Number of Posts to Show', 'power' ); ?>:</label>
			...
			<div class="power-widget-column-box">
				...
				<p>
					...
					<label for="<?php echo esc_attr( $this->get_field_id( 'more_from_category' ) ); ?>"><?php esc_html_e( 'Show Category Archive Link', 'power' ); ?></label>
				</p>

				<p>
					<label for="<?php echo esc_attr( $this->get_field_id( 'more_from_category_text' ) ); ?>"><?php esc_html_e( 'Link Text', 'power' ); ?>:</label>
