/**
 * Provides a PowerImageToggleInfo component that displays the state
...
class ImageToggleInfo extends Component {
	 ...
	 */
	componentDidMount() {
		apiFetch( { path: '/power/v1/singular-images' } ).then( ( types ) => {
	...
	/**
	 * Gives current state of the “show featured images on…” Power setting
	...
	updateSingularImagesSetting( newValue ) {
		...
		apiFetch( putRequest ).then( ( types ) => {
			...
			let statusAnnouncement = sprintf( __( 'Featured images now enabled on %s.', 'power' ), this.props.currentPostTypeLabel );

			if ( this.postTypeHasSingularImagesDisabled() ) {
				/* translators: %s: the current page type, such as “Pages” or “Posts”. */
				statusAnnouncement = sprintf( __( 'Featured images now disabled on %s.', 'power' ), this.props.currentPostTypeLabel );
			}
	...
	render() {
		...
		if ( this.state.updating ) {
			/* translators: %s: the current page type, such as “Pages” or “Posts”. */
			let label = sprintf( __( 'Disabling images on %s...', 'power' ), this.props.currentPostTypeLabel );

			if ( this.postTypeHasSingularImagesDisabled() ) {
				/* translators: %s: the current page type, such as “Pages” or “Posts”. */
				label = sprintf( __( 'Enabling images on %s...', 'power' ), this.props.currentPostTypeLabel );
			}
		...
		/* translators: %s: the current page type, such as “Pages” or “Posts”. */
		let statusText = sprintf( __( 'Featured images are enabled on %s. ', 'power' ), this.props.currentPostTypeLabel );
		/* translators: %s: the current page type, such as “Pages” or “Posts”. */
		let ariaText = sprintf( __( 'Disable featured images on all %s?' ), this.props.currentPostTypeLabel );
		let linkText = __( 'Disable images.', 'power' );
		let newImageState = 0; // Turn featured images off.

		if ( this.postTypeHasSingularImagesDisabled() ) {
			textClass = 'power-sidebar-label-disabled';
			/* translators: %s: the current page type, such as “Pages” or “Posts”. */
			statusText = sprintf( __( 'Featured images are disabled on %s.', 'power' ), this.props.currentPostTypeLabel );
			/* translators: %s: the current page type, such as “Pages” or “Posts”. */
			ariaText = sprintf( __( 'Enable featured images on all %s' ), this.props.currentPostTypeLabel );
			linkText = __( 'Enable images.', 'power' );
...
export const PowerImageToggleInfo = compose( [
	withSelect( () => {
