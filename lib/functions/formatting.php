 ...
 */
function power_truncate_phrase( $text, $max_characters ) {
...
// phpcs:disable WordPress.NamingConventions.PrefixAllGlobals.NonPrefixedFunctionFound, WordPress.NamingConventions.PrefixAllGlobals.NonPrefixedHooknameFound -- Some of the earliest Power functions. Can't be renamed.
/**
...
function get_the_content_limit( $max_characters, $more_link_text = '(more...)', $stripteaser = false ) {
	...
	$content = power_truncate_phrase( $content, $max_characters );
 ...
 */
function power_a11y_more_link( $more_link_text ) {

	if ( ! empty( $more_link_text ) && power_a11y( 'screen-reader-text' ) ) {
		$more_link_text .= ' <span class="screen-reader-text">' . __( 'about ', 'power' ) . get_the_title() . '</span>';
	}
 ...
 */
function power_rel_nofollow( $text ) {

	$text = power_strip_attr( $text, 'a', 'rel' );
 ...
 * @since 1.0.0
 *
 * @link http://coreengine.com/support/showthread.php?t=20633
 *
 ...
 */
function power_strip_attr( $text, $elements, $attributes, $two_passes = true ) {
 ...
 */
function power_paged_post_url( $i, $post_id = 0 ) {
	global $wp_rewrite;
 ...
 */
function power_sanitize_html_classes( $classes, $return_format = 'input' ) {
 ...
 */
function power_formatting_allowedtags() {

	return apply_filters(
		'power_formatting_allowedtags',
		[
			'a'          => [
				'href'  => [],
				'title' => [],
 ...
 */
function power_formatting_kses( $string ) {

	return wp_kses( $string, power_formatting_allowedtags() );
 ...
 */
function power_human_time_diff( $older_date, $newer_date = false, $relative_depth = 2 ) {
	...
	if ( ! $since ) {
		return '0 ' . _x( 'seconds', 'time difference', 'power' );
	}

	// Hold units of time in seconds, and their pluralised strings (not translated yet).
	$units = [
		/* translators: %s: Number of year(s). */
		[ 31536000, _nx_noop( '%s year', '%s years', 'time difference', 'power' ) ],  // 60 * 60 * 24 * 365
		/* translators: %s: Number of month(s). */
		[ 2592000, _nx_noop( '%s month', '%s months', 'time difference', 'power' ) ], // 60 * 60 * 24 * 30
		/* translators: %s: Number of week(s). */
		[ 604800, _nx_noop( '%s week', '%s weeks', 'time difference', 'power' ) ],    // 60 * 60 * 24 * 7
		/* translators: %s: Number of day(s). */
		[ 86400, _nx_noop( '%s day', '%s days', 'time difference', 'power' ) ],       // 60 * 60 * 24
		/* translators: %s: Number of hour(s). */
		[ 3600, _nx_noop( '%s hour', '%s hours', 'time difference', 'power' ) ],      // 60 * 60
		/* translators: %s: Number of minute(s). */
		[ 60, _nx_noop( '%s minute', '%s minutes', 'time difference', 'power' ) ],
		/* translators: %s: Number of second(s). */
		[ 1, _nx_noop( '%s second', '%s seconds', 'time difference', 'power' ) ],
	];
	...
	while ( $amount_date_partials < $relative_depth && $i < $amount_units ) {
		...
		if ( 0 !== $count ) {
			$date_partials[]      = sprintf( translate_nooped_plural( $units[ $i ][1], $count, 'power' ), $count );
	...
	if ( empty( $date_partials ) ) {
		...
		// Add 'and' separator.
		$output .= ' ' . _x( 'and', 'separator in time difference', 'power' ) . ' ';
 ...
 *
 * Used almost exclusively within labels and text in user interfaces added by Power.
 *
 * @since 2.0.0
 *
 ...
 */
function power_strip_p_tags( $content ) {
