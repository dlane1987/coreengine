 ...
 *
 * Applies `power_pre_get_option_$key` and `power_options` filters.
 *
 ...
 * @param string $setting   Optional. Settings field name. Eventually defaults to `POWER_SETTINGS_FIELD` if not
 *                          passed as an argument.
 * @param bool   $use_cache Optional. Whether to use the Power cache value or not. Default is true.
 * @return mixed The value of the `$key` in the database, or the return from
 *               `power_pre_get_option_{$key}` short circuit filter if not `null`.
 */
function power_get_option( $key, $setting = null, $use_cache = true ) {

	// The default is set here, so it doesn't have to be repeated in the function arguments for power_option() too.
	$setting = $setting ?: POWER_SETTINGS_FIELD;

	// Allow child theme to short circuit this function.
	$pre = apply_filters( "power_pre_get_option_{$key}", null, $setting );
	if ( null !== $pre ) {
		return $pre;
	}

	// Bypass cache if viewing site in Customizer.
	if ( power_is_customizer() ) {
		$use_cache = false;
	}
	...
	if ( isset( $settings_cache[ $setting ] ) ) {
		...
		$options = apply_filters( 'power_options', $settings_cache[ $setting ], $setting );
	} else {
		// Set value and cache setting.
		$settings_cache[ $setting ] = apply_filters( 'power_options', get_option( $setting ), $setting );
 ...
 *
 * @param string $key       Option name.
 * @param string $setting   Optional. Settings field name. Eventually defaults to POWER_SETTINGS_FIELD.
 * @param bool   $use_cache Optional. Whether to use the Power cache value or not. Default is true.
 */
function power_option( $key, $setting = null, $use_cache = true ) {

	// phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped
	echo power_get_option( $key, $setting, $use_cache );
 ...
 *
 * @param string $key       Option name.
 * @param bool   $use_cache Optional. Whether to use the Power cache value or not. Defaults to true.
 * @return mixed The value of the `$key` in the database, or the return from
 *               `power_pre_get_option_{$key}` short circuit filter if not `null`.
 */
function power_get_seo_option( $key, $use_cache = true ) {

	return power_get_option( $key, POWER_SEO_SETTINGS_FIELD, $use_cache );
 ...
 * @param bool   $use_cache Optional. Whether to use the Power cache value or not. Defaults to true.
 */
function power_seo_option( $key, $use_cache = true ) {

	power_option( $key, POWER_SEO_SETTINGS_FIELD, $use_cache );
 ...
 *               `power_pre_get_option_{$key}` short circuit filter if not `null`.
 */
function power_get_cpt_option( $key, $post_type_name = '', $use_cache = true ) {

	$post_type_name = power_get_global_post_type_name( $post_type_name );

	return power_get_option( $key, POWER_CPT_ARCHIVE_SETTINGS_FIELD_PREFIX . $post_type_name, $use_cache );
 ...
 * @param bool   $use_cache      Optional. Whether to use the Power cache value or not. Defaults to true.
 */
function power_cpt_option( $key, $post_type_name, $use_cache = true ) {

	// phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped
	echo power_get_cpt_option( $key, $post_type_name, $use_cache );
 ...
 */
function power_custom_field( $field, $output_pattern = '%s', $post_id = null ) {

	$value = power_get_custom_field( $field, $post_id );
	if ( $value ) {
		// phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped
 ...
 */
function power_get_custom_field( $field, $post_id = null ) {
 ...
 */
function power_save_custom_fields( array $data, $nonce_action, $nonce_name, $post, $deprecated = null ) {
 ...
 */
function power_get_expiring_setting( $setting, $current_time = null ) {
	if ( is_null( $current_time ) ) {
		$current_time = time();
	}

	// Prefix the setting, so we're not messing with non-expiring settings.
	$setting = 'power_expiring_setting_' . $setting;
 ...
 */
function power_set_expiring_setting( $setting, $value, $duration ) {
	// Prefix the setting, so we're not messing with non-expiring settings.
	$setting = 'power_expiring_setting_' . $setting;
 ...
 */
function power_delete_expiring_setting( $setting ) {
	// Prefix the setting, so we're not messing with non-expiring settings.
	$setting = 'power_expiring_setting_' . $setting;
 ...
 */
function power_update_settings( $new = '', $setting = POWER_SETTINGS_FIELD ) {
