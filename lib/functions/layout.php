...
add_action( 'power_setup', 'power_create_initial_layouts' );
/**
 * Register Power default layouts.
 *
 * Power comes with six layouts registered by default. These are:
 *
 ...
 * @since 1.4.0
 */
function power_create_initial_layouts() {

	// Common path to default layout images.
	$url = POWER_ADMIN_IMAGES_URL . '/layouts/';

	$layouts = apply_filters( 'power_initial_layouts', power_get_config( 'layouts' ), $url );

	foreach ( (array) $layouts as $layout_id => $layout_args ) {
		power_register_layout( $layout_id, $layout_args );
	}

}

/**
 * Register new layouts in Power.
 *
 * Modifies the global `$_power_layouts` variable.
 *
 ...
 * `power_set_default_layout()` function to ensure only one layout is set as the default at one time.
 *
 * @since 1.4.0
 *
 * @see power_set_default_layout() Set a default layout.
 *
 * @global array $_power_layouts Holds all layouts data.
 *
 ...
 */
function power_register_layout( $id = '', $args = [] ) {

	global $_power_layouts;

	if ( ! is_array( $_power_layouts ) ) {
		$_power_layouts = [];
	}

	// Don't allow empty $id, or double registrations.
	if ( ! $id || isset( $_power_layouts[ $id ] ) ) {
		return false;
	}

	$defaults = [
		'label' => __( 'No Label Selected', 'power' ),
		'img'   => POWER_ADMIN_IMAGES_URL . '/layouts/none.gif',
		'type'  => [ 'site' ],
	];

	$args = wp_parse_args( $args, $defaults );

	$_power_layouts[ $id ] = $args;
 ...
 */
function power_add_type_to_layout( $id, $type = [] ) {

	global $_power_layouts;

	$new_type = array_merge( (array) $_power_layouts[ $id ]['type'], (array) $type );

	$_power_layouts[ $id ]['type'] = $new_type;
 ...
 */
function power_remove_type_from_layout( $id, $type = [] ) {

	global $_power_layouts;

	$new_type = array_values( array_diff( (array) $_power_layouts[ $id ]['type'], (array) $type ) );
 ...
 */
function power_set_default_layout( $id = '' ) {
	...
	foreach ( $_power_layouts as $key => $value ) {
		if ( isset( $_power_layouts[ $key ]['default'] ) ) {
			unset( $_power_layouts[ $key ]['default'] );
		}
	}

	$_power_layouts[ $id ]['default'] = true;
...
/**
 * Unregister a layout in Power.
 *
 * Modifies the global $_power_layouts variable.
 *
 * @since 1.4.0
 *
 * @global array $_power_layouts Holds all layout data.
 *
 ...
 */
function power_unregister_layout( $id = '' ) {
...
/**
 * Return all registered Power layouts.
 *
 * @since 1.4.0
 *
 * @global array $_power_layouts Holds all layout data.
 ...
 */
function power_get_layouts( $type = 'site' ) {
	...
	foreach ( $types as $type ) {
		foreach ( $_power_layouts as $id => $data ) {
	 ...
	 */
	$layouts = (array) apply_filters( 'power_get_layouts', $layouts, $type );
 ...
 * @return array Registered layouts.
 */
function power_get_layouts_for_customizer( $type = 'site' ) {

	$layouts = power_get_layouts( $type );
 ...
 */
function power_get_layout( $id, $type = 'site' ) {
 ...
 */
function power_get_default_layout( $type = 'site' ) {
 ...
 */
function power_has_multiple_layouts( $type = 'site' ) {
 ...
 *
 * Applies `power_site_layout` filter early to allow shortcutting of function.
 *
 * @since 1.0.0
 *
 ...
 * @return string Key of site layout or filtered value of `power_site_layout`.
 */
function power_site_layout( $use_cache = true ) {

	// Allow child theme to short-circuit this function.
	$pre = apply_filters( 'power_site_layout', null );
	if ( null !== $pre ) {
		return $pre;
	}
	...
	if ( is_singular() || ( is_home() && ! power_is_root_page() ) ) { // If viewing a singular page or post, or the posts page, but not the front page.

		$post_id      = is_home() ? get_option( 'page_for_posts' ) : null;
		$custom_field = power_get_custom_field( '_power_layout', $post_id );
		$site_layout  = $custom_field ?: power_get_option( 'site_layout' );
		...
		$site_layout = $term_layout ?: power_get_option( 'site_layout' );
		$type        = [ 'archive', $term->taxonomy, $term->term_id ];

	} elseif ( is_post_type_archive() && power_has_post_type_archive_support() ) { // If viewing a supported post type.

		$site_layout = power_get_cpt_option( 'layout' ) ?: power_get_option( 'site_layout' );
		$type        = [ 'archive', 'post-type-archive-' . get_post_type() ];

	} elseif ( is_author() ) { // If viewing an author archive.

		$site_layout = get_the_author_meta( 'layout', (int) get_query_var( 'author' ) ) ?: power_get_option( 'site_layout' );
		$type        = [ 'archive', 'author', get_query_var( 'author' ) ];

	} else { // Else pull the theme option.

		$site_layout = power_get_option( 'site_layout' );

	}

	// Use default layout as a fallback, if necessary.
	if ( ! power_get_layout( $site_layout, $type ) ) {
		$site_layout = power_get_default_layout();
	}
 ...
 *
 * You must manually wrap this in an HTML element with the class of `power-layout-selector` in order for the CSS and
 ...
 *
 * The Power admin script is enqueued to ensure the layout selector behaviour (amending label class to add border on
 ...
 */
function power_layout_selector( $args = [] ) {
	...
	foreach ( power_get_layouts( $args['type'] ) as $id => $data ) {
 ...
 *
 * A check is made to see if the `$context` is in the `power-structural-wraps` theme support data. If so, then the
 ...
 * @return null|string Wrap HTML, or `null` if `power-structural-wraps` support is falsy.
 */
function power_get_structural_wrap( $context = '', $output = 'open' ) {

	$wraps = get_theme_support( 'power-structural-wraps' );
	...
	switch ( $output ) {
		case 'open':
			$output = sprintf( '<div %s>', power_attr( 'structural-wrap' ) );
			break;
		case 'close':
			$output = '</div>';
			break;
	...
	$output = apply_filters( "power_structural_wrap-{$context}", $output, $original_output ); // phpcs:ignore WordPress.NamingConventions.ValidHookName.UseUnderscores -- Power way of dynamic filters.
 ...
 *
 * @since 1.6.0
 * @since 2.7.0 Logic moved to `power_get_structural_wrap()` and third parameter deprecated.
 *
 * @param string $context    The location ID.
 * @param string $output     Optional. The markup to include. Can also be 'open'
 *                           (default) or 'close' to use pre-determined markup for consistency.
 ...
 * @return null|string Wrap HTML, or `null` if `power-structural-wraps` support is falsy.
 */
function power_structural_wrap( $context = '', $output = 'open', $deprecated = null ) {

	if ( null !== $deprecated ) {
		$message = __( 'The default is true, so remove the third argument.', 'power' );

		if ( false === (bool) $deprecated ) {
			$message = __( 'Use `power_get_structural_wrap()` instead.', 'power' );
		}

		_deprecated_argument( __FUNCTION__, '2.7.0', esc_html( $message ) );
	}

	$output = power_get_structural_wrap( $context, $output );
...
// phpcs:disable PHPCompatibility.FunctionNameRestrictions.ReservedFunctionNames.FunctionDoubleUnderscore, WordPress.NamingConventions.ValidFunctionName.FunctionDoubleUnderscore, WordPress.NamingConventions.PrefixAllGlobals.NonPrefixedFunctionFound -- Highly unlikely PHP will start using __power.
 ...
 */
function __power_return_content_sidebar() {
 ...
 */
function __power_return_sidebar_content() {
 ...
 */
function __power_return_content_sidebar_sidebar() {
 ...
 */
function __power_return_sidebar_sidebar_content() {
 ...
 */
function __power_return_sidebar_content_sidebar() {
 ...
 */
function __power_return_full_width_content() {
