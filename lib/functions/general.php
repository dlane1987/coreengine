 ...
 */
function power_enable_author_box( $args = [] ) {
	...
	if ( 'single' === $args['type'] ) {
		add_filter( 'get_the_author_power_author_box_single', '__return_true' );
	} elseif ( 'archive' === $args['type'] ) {
		add_filter( 'get_the_author_power_author_box_archive', '__return_true' );
	}
 ...
 */
function power_admin_redirect( $page, array $query_args = [] ) {
...
add_action( 'template_redirect', 'power_custom_field_redirect', 20 );
/**
 ...
 */
function power_custom_field_redirect() {

	if ( ! is_singular() ) {
		return;
	}

	$url = power_get_custom_field( 'redirect' );
	if ( $url ) {
 ...
 */
function power_get_theme_support_arg( $feature, $arg, $default = '' ) {
 ...
 */
function power_get_theme_handle() {
 ...
 */
function power_get_theme_version() {
 ...
 */
function power_get_config( $config ) {
 ...
 */
function power_detect_plugin( array $plugins ) {
...
/**
 * Check that we're targeting a specific Power admin page.
 *
 * The `$pagehook` argument is expected to be one of 'power', 'seo-settings' or 'power-import-export' although
 ...
 */
function power_is_menu_page( $pagehook = '' ) {
 ...
 */
function power_is_blog_template() {
 ...
 */
function power_get_global_post_type_name( $post_type_name = '' ) {
 ...
 * - support "power-cpt-archives-settings".
 *
 * This last item means that if you're using an archive template and don't want Power interfering with it with these
 * archive settings, then don't add the support. This support check is handled in
 * {@link power_has_post_type_archive_support()}.
 *
 * Applies the `power_cpt_archives_args` filter, to change the conditions for which post types are deemed valid.
 *
 ...
 */
function power_get_cpt_archive_types() {

	static $power_cpt_archive_types;
	if ( $power_cpt_archive_types ) {
		return $power_cpt_archive_types;
	}

	$args = apply_filters(
		'power_cpt_archives_args',
		[
			'public'       => true,
			'show_ui'      => true,
			'show_in_menu' => true,
	...
	$power_cpt_archive_types = get_post_types( $args, 'objects' );
 ...
 */
function power_get_cpt_archive_types_names() {

	$post_type_names = [];
	foreach ( power_get_cpt_archive_types() as $post_type ) {
 ...
 *
 * @param string $post_type_name Post type name.
 * @return bool `true` if custom post type name has `power-cpt-archives-settings` support, `false` otherwise.
 */
function power_has_post_type_archive_support( $post_type_name = '' ) {

	$post_type_name = power_get_global_post_type_name( $post_type_name );

	return in_array( $post_type_name, power_get_cpt_archive_types_names(), true ) &&
		post_type_supports( $post_type_name, 'power-cpt-archives-settings' );
...
/**
 * Determine if theme support power-accessibility is activated by the child theme.
 ...
 * @return bool `true` if current theme supports `power-accessibility`, or a specific feature of it, `false` otherwise.
 */
function power_a11y( $arg = 'screen-reader-text' ) {

	$feature = 'power-accessibility';
 ...
 *
 * @see power_get_sitemap()
 *
 * @since 2.2.0
 *
 * @param string $heading Optional. Heading element. Default is `h2`.
 */
function power_sitemap( $heading = 'h2' ) {

	echo wp_kses_post( power_get_sitemap( $heading ) );
 ...
 */
function power_get_sitemap( $heading = 'h2' ) {
	 ...
	 */
	$pre = apply_filters( 'power_pre_get_sitemap', null );
	if ( null !== $pre ) {
		return $pre;
	}

	$sitemap  = sprintf( '<%2$s>%1$s</%2$s>', __( 'Pages:', 'power' ), $heading );
	$sitemap .= sprintf( '<ul>%s</ul>', wp_list_pages( 'title_li=&echo=0' ) );

	$post_counts = wp_count_posts();
	if ( $post_counts->publish > 0 ) {
		$sitemap .= sprintf( '<%2$s>%1$s</%2$s>', __( 'Categories:', 'power' ), $heading );
		$sitemap .= sprintf( '<ul>%s</ul>', wp_list_categories( 'sort_column=name&title_li=&echo=0' ) );

		$sitemap .= sprintf( '<%2$s>%1$s</%2$s>', __( 'Authors:', 'power' ), $heading );
		$sitemap .= sprintf( '<ul>%s</ul>', wp_list_authors( 'exclude_admin=0&optioncount=1&echo=0' ) );

		$sitemap .= sprintf( '<%2$s>%1$s</%2$s>', __( 'Monthly:', 'power' ), $heading );
		$sitemap .= sprintf( '<ul>%s</ul>', wp_get_archives( 'type=monthly&echo=0' ) );

		$sitemap .= sprintf( '<%2$s>%1$s</%2$s>', __( 'Recent Posts:', 'power' ), $heading );
	 ...
	 */
	return apply_filters( 'power_sitemap_output', $sitemap );
 ...
 */
function power_plugin_install_link( $plugin_slug = '', $text = '' ) {
...
function power_canonical_url() {
	...
	if ( is_singular() ) {
		...
		$cf = power_get_custom_field( '_power_canonical_uri' );

		if ( $cf ) {
			$canonical = $cf;
		} elseif ( $numpages > 1 && $page > 1 ) {
			$canonical = power_paged_post_url( $page, $id );
		} else {
	...
	return apply_filters( 'power_canonical_url', $canonical );
