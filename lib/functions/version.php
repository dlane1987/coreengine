...
/**
 * Calculate or return the first version of Power to run on this site.
 *
 * @since 2.1.0
 *
 * @return string First version of Power to run on the site.
 */
function power_first_version() {
	$first_version = power_get_option( 'first_version' );
 ...
 */
function power_first_version_compare( $version, $operator ) {
	return version_compare( power_first_version(), $version, $operator );
}
 ...
 */
function power_get_db_version( $cache = false ) {
	$db_version = power_get_option( 'db_version', null, $cache );
	...
	$db_version = str_replace(
		'.',
		'',
		(string) power_get_option( 'theme_version', null, $cache ) ?: (string) ( (int) PARENT_DB_VERSION - 1 )
	);
