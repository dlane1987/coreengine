...
add_filter( 'feed_link', 'power_feed_links_filter', 10, 2 );
/**
 ...
 */
function power_feed_links_filter( $output, $feed ) {

	$feed_uri          = power_get_option( 'feed_uri' );
	$comments_feed_uri = power_get_option( 'comments_feed_uri' );
...
add_action( 'template_redirect', 'power_feed_redirect' );
/**
...
function power_feed_redirect() {
	...
	if ( $feed_uri && ! is_comment_feed() && power_get_option( 'redirect_feed' ) ) {
		wp_redirect( $feed_uri, 302 ); // phpcs:ignore WordPress.Security.SafeRedirect.wp_redirect_wp_redirect -- User inputs redirect URL.
		exit;
	}

	if ( $comments_feed_uri && is_comment_feed() && power_get_option( 'redirect_comments_feed' ) ) {
