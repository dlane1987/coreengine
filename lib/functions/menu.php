...
/**
 * Determine if a child theme supports a particular Power nav menu.
 *
 * @since 1.8.0
 *
 ...
 */
function power_nav_menu_supported( $menu ) {
 ...
 * `power_superfish_enabled` filter is true, then this function returns true. False otherwise.
 *
 * @since 1.9.0
 *
 * @return bool `true` if Superfish is enabled, `false` otherwise.
 */
function power_superfish_enabled() {
	...
	return ( power_a11y( 'drop-down-menu' ) || apply_filters( 'power_superfish_enabled', false ) );
 ...
 *
 * @return Power_Menu_Handler returns an instance of the handler.
 */
function power_register_responsive_menus( array $config = [] ) {

	static $menu_handler = null;

	if ( null === $menu_handler && ! empty( $config ) ) {

		$menu_handler = new Power_Menu_Handler( get_stylesheet(), $config );
...
/**
 * Return the markup to display a menu consistent with the Power format.
 *
 * Applies the `power_$location_nav` filter e.g. `power_header_nav`. For primary and secondary menu locations, it
 * applies the `power_do_nav` and `power_do_subnav` filters instead for backwards compatibility.
 *
 * @since 2.1.0
 *
 ...
 */
function power_get_nav_menu( $args = [] ) {
	...
	$args = wp_parse_args(
		...
		[
			'theme_location' => '',
			'container'      => '',
			'menu_class'     => 'menu power-nav-menu',
			'link_before'    => power_markup(
				[
					'open'    => '<span %s>',
	...
	// If power-accessibility for 'drop-down-menu' is enabled and the menu doesn't already have the superfish class, add it.
	if ( power_superfish_enabled() && false === strpos( $args['menu_class'], 'js-superfish' ) ) {
	...
	$nav_markup_open  = power_get_structural_wrap( 'menu-' . $sanitized_location, 'open' );
	$nav_markup_close = power_get_structural_wrap( 'menu-' . $sanitized_location, 'close' );

	$params = [
		'theme_location' => $args['theme_location'],
	];

	$nav_output = power_markup(
		[
			'open'    => '<nav %s>',
			'close'   => '</nav>',
	 ...
	 * }
	 */
	return apply_filters( "power_{$filter_location}", $nav_output, $nav, $args );
}

/**
 * Echo the output from `power_get_nav_menu()`.
 *
 * @since 2.1.0
 *
 * @param string|array $args Menu arguments.
 */
function power_nav_menu( $args ) {
 ...
 */
function power_unique_menu_name( $name ) {
