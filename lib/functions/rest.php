...
namespace CoreEngine\Power\Functions\Rest;

add_action( 'rest_api_init', __NAMESPACE__ . '\\layouts' );
/**
 * Add `layouts` endpoint to the REST API.
 *
 * Example: `curl https://example.com/wp-json/power/v1/layouts/all`
 *
 * @since 3.1.0
 */
function layouts() {
 ...
 *
 * Returns an array of post types that have power-singular-images support
 * with singular images enabled in the Singular Content panel in the Customizer.
 *
 * Example: `curl https://example.com/wp-json/power/v1/singular-images`
 *
 * Example response:
 *
 * [
...
function get_singular_images() {
	\register_rest_route(
		...
		[
			'methods'  => 'GET',
			'callback' => '\power_post_types_with_singular_images_enabled',
		]
	);
}
 ...
 * types have `power-singular-images` support:
 *
 * [ "posts" ]
 *
 * @since 3.1.0
		...
		[
			...
			'callback'            => function( $request ) {
				...
				foreach ( $post_types as $type => $value ) {
					...
					$power_options[ "show_featured_image_{$type}" ] = $value;
					update_option( POWER_SETTINGS_FIELD, $power_options );
				}

				return \power_post_types_with_singular_images_enabled();
 ...
 *
 * Example: `curl https://example.com/wp-json/power/v1/breadcrumbs`
 *
...
function get_breadcrumbs() {
	\register_rest_route(
		...
		[
			'methods'  => 'GET',
			'callback' => '\power_breadcrumb_options_enabled',
		]
	);
}
		...
		[
			...
			'callback'            => function( $request ) {
				...
				foreach ( $types as $type => $value ) {
					...
					$power_options[ $type ] = $value;
				}

				update_option( POWER_SETTINGS_FIELD, $power_options );

				return \power_breadcrumb_options_enabled();
 ...
 *
 * Example: `curl https://example.com/wp-json/power/v1/reading-settings`
 *
 * Example response:
 *
